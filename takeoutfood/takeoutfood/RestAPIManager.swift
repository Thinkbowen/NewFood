...
//        }.resume()
  ...
  //For payment request
  func post_payment(payment:Payment,
                    onSuccess:@escaping(Receipt)->Void,
                    onFailure:@escaping(Any)->Void){
    guard let url = URL(string:("https://api.na.bambora.com/v1/payments"))else{return}
    var request = URLRequest(url:url)
    request.httpMethod = "POST"
    request.addValue("Passcode MzAwMjA1MjEwOjViNjYyQWQ1NzgzMzQwMDY4ODFDYmNiNGUwNmI5MDMx", forHTTPHeaderField: "Authorization")
    request.addValue("application/json", forHTTPHeaderField: "Content-Type")
    do{
      let jsonBody = try JSONEncoder().encode(payment)
      request.httpBody = jsonBody
    }catch{
      print("payment encode fail")
    }
    getRequestObject(request: request, onSuccess: onSuccess, onFailure: onFailure)
  }
